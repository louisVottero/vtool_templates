...
def main():
    ...
    for side in ['L','R']:
        ...
        joints_list = process.get_runtime_value('top_joints_%s' % side)
        lid_control = 'CNT_UPPER_EYELID_1_%s' % side
        description = 'eyelid_top'
...
def create_lid_controls(main_control, joints_list, description, side):
    ...
    rig.create_control_follow(main_control, 2, .9)
    ...
    cmds.parent(rig.control_group, 'controls_upper_eyelid_1_%s' % side)
