...
def main():
    ...
    if cmds.upAxis(q = True, ax = True) == 'z':    
        top_control.translate_shape(0,-1*size,10*size)    
    else:
    ...
    cmds.orientConstraint(put.control_sub_head, 'locator_rivet_%s' % put.joint_head, mo = True)                            
    
    if cmds.objExists('GDE_headPivot'):
    ...
    cmds.parent('xform_%s' % put.control_settings, put.control_head)
